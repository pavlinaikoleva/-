...
public:
	...
	//уволн€ване на работник или премахване на екип
	void fireEmployee(const char * _EGN);
	//работа с файлове
	void SaveCompanyOnFile(std::ofstream& out);
	void loadEmployee(std::ifstream& input);
	void loadCompanyFromFile(std::ifstream& input);
	//деструктор
	~Company();
};
#endif