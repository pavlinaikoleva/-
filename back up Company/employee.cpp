...
Employee::Employee(const char * _name, const char * _address, const char * _EGN,const Date& _date, Employee * _boss)
{
	strcpy_s(name, strlen(_name) + 1, _name);
	strcpy_s(address, strlen(_address) + 1, _address);
	strcpy_s(EGN, strlen(_EGN) + 1, _EGN);
...
std::ostream& operator << (std::ostream &out, std::vector<Employee*> employees)
{
